      ...
      <Box sx={{ p: { xs: 1.5, md: 2 } }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.2, minWidth: 0 }}>
            ...
            <Box sx={{ minWidth: 0 }}>
              ...
              <Typography sx={{ color: palette.muted, fontSize: "11.5px", mt: 0.25 }}>
                Conductores y licencias habilitadas para transporte.
              </Typography>
            </Box>
          </Box>
        ...
        <Grid container spacing={1.15}>
          <Grid item xs={12} md={3}>
            <Field label="Licencia" icon={<Hash size={15} />}>
          ...
          <Grid item xs={12}>
            <Field label="Descripcion" icon={<FileText size={15} />}>
              <CaptureInput value={draft.descripcion} onChange={(value) => updateDraft("descripcion", value)} placeholder="Observaciones, categoria o vigencia" />
            </Field>
          </Grid>
        </Grid>
...
export default function TrLicenciaList() {
  ...
  const cargarDatos = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`${back_host}/mve_translicencia/${params.id_anfitrion}/${params.documento_id}`);
      const result = await response.json();
      const data = Array.isArray(result?.data) ? result.data : [];
      setRows(data);
      setBaseRows(data);
    } catch (error) {
      console.log("Error cargando licencias:", error);
      setRows([]);
      setBaseRows([]);
    } finally {
      setLoading(false);
  ...
  const guardar = async (draft) => {
    ...
    try {
      const response = await fetch(`${back_host}/mve_translicencia`, {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.message || "No se pudo guardar la licencia.");
      }
      setModalOpen(false);
      setEditando(null);
      cargarDatos();
  ...
  const eliminar = async (row) => {
    ...
    try {
      const response = await fetch(`${back_host}/mve_translicencia/${params.id_anfitrion}/${params.documento_id}/${row.licencia}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok || !data.success) {
        throw new Error(data.message || "No se pudo eliminar la licencia.");
      }
      cargarDatos();
    } catch (error) {
    ...
    <Box sx={{ minHeight: "100vh", backgroundColor: palette.bg, p: { xs: 2, md: 4 } }}>
      <Box sx={{ maxWidth: 1080, mx: "auto" }}>
        <Box sx={{ display: "flex", alignItems: { xs: "stretch", sm: "center" }, justifyContent: "space-between", gap: 1.2, mb: 2, flexDirection: { xs: "column", sm: "row" } }}>
          <Box>
            <Typography sx={{ color: palette.text, fontWeight: 800, fontSize: "22px", lineHeight: 1.2 }}>Licencias</Typography>
            <Typography sx={{ color: palette.muted, fontSize: "13px", mt: 0.5 }}>{rows.length} licencias registradas</Typography>
          </Box>
          <AppSearch placeholder="Buscar licencia..." value={busqueda} onChange={(event) => { setBusqueda(event.target.value); filtrar(event.target.value); }} />
        ...
        <DataTable
          ...
          noDataComponent={<Box sx={{ py: 4, color: palette.muted, display: "flex", alignItems: "center", gap: 1 }}><Search size={16} />Sin licencias registradas</Box>}
        />
      </Box>

      <LicenciaModal open={modalOpen} licencia={editando} onClose={() => { setModalOpen(false); setEditando(null); }} onSubmit={guardar} />
    </Box>
  );
}
