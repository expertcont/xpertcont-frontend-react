...
function PlacaModal({ open, placa, onClose, onSubmit }) {
      ...
      <Box sx={{ p: { xs: 1.5, md: 2 } }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.2, minWidth: 0 }}>
            ...
            <Box sx={{ minWidth: 0 }}>
              <Typography sx={{ fontWeight: 800, fontSize: { xs: "16px", md: "17px" }, lineHeight: 1.2 }}>
                {placa ? "Editar placa" : "Nueva placa"}
              </Typography>
              <Typography sx={{ color: palette.muted, fontSize: "11.5px", mt: 0.25 }}>
                Unidades habilitadas para operaciones de transporte.
              </Typography>
            </Box>
          </Box>
        ...
        <Grid container spacing={1.15}>
          <Grid item xs={12} md={4}>
            <Field label="Placa" icon={<Hash size={15} />}>
              <CaptureInput
                value={draft.placa}
                onChange={(value) => updateDraft("placa", value.toUpperCase())}
                placeholder="V1A-123"
                readOnly={Boolean(placa)}
              />
            </Field>
          </Grid>
          <Grid item xs={12} md={4}>
            <Field label="Marca" icon={<Truck size={15} />}>
              <CaptureInput value={draft.marca} onChange={(value) => updateDraft("marca", value.toUpperCase())} placeholder="VOLVO" />
            </Field>
          </Grid>
          <Grid item xs={12} md={4}>
            <Field label="Certificado" icon={<ShieldCheck size={15} />}>
...
export default function TrPlacaList() {
  ...
  const cargarDatos = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`${back_host}/mve_transplaca/${params.id_anfitrion}/${params.documento_id}`);
      const result = await response.json();
      const data = Array.isArray(result?.data) ? result.data : [];
      setRows(data);
      setBaseRows(data);
    } catch (error) {
      console.log("Error cargando placas:", error);
      setRows([]);
      setBaseRows([]);
    } finally {
      setLoading(false);
  ...
  const guardar = async (draft) => {
    ...
    try {
      const response = await fetch(`${back_host}/mve_transplaca`, {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.message || "No se pudo guardar la placa.");
      }
      setModalOpen(false);
      setEditando(null);
      cargarDatos();
  ...
  const eliminar = async (row) => {
    const result = await confirmDialog({
      ...
      message: `${row.placa}${row.marca ? ` - ${row.marca}` : ""}`,
      icon: "warning",
    ...
    try {
      const response = await fetch(`${back_host}/mve_transplaca/${params.id_anfitrion}/${params.documento_id}/${row.placa}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok || !data.success) {
        throw new Error(data.message || "No se pudo eliminar la placa.");
      }
      cargarDatos();
    } catch (error) {
      ...
      cell: row => (
        <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 0.7, width: "100%" }}>
          <Tooltip title="Editar" arrow>
            <Box sx={actionButtonSx()} onClick={() => { setEditando(row); setModalOpen(true); }}>
              <Pencil size={14} />
            </Box>
          </Tooltip>
          <Tooltip title="Eliminar" arrow>
            <Box sx={actionButtonSx(true)} onClick={() => eliminar(row)}>
              <Trash2 size={14} />
            </Box>
          </Tooltip>
        </Box>
    ...
    <Box sx={{ minHeight: "100vh", backgroundColor: palette.bg, p: { xs: 2, md: 4 } }}>
      <Box sx={{ maxWidth: 980, mx: "auto" }}>
        <Box sx={{ display: "flex", alignItems: { xs: "stretch", sm: "center" }, justifyContent: "space-between", gap: 1.2, mb: 2, flexDirection: { xs: "column", sm: "row" } }}>
          <Box>
            <Typography sx={{ color: palette.text, fontWeight: 800, fontSize: "22px", lineHeight: 1.2 }}>Placas</Typography>
            <Typography sx={{ color: palette.muted, fontSize: "13px", mt: 0.5 }}>{rows.length} placas registradas</Typography>
          </Box>
          <AppSearch placeholder="Buscar placa..." value={busqueda} onChange={(event) => { setBusqueda(event.target.value); filtrar(event.target.value); }} />
        ...
        <DataTable
          ...
          noDataComponent={<Box sx={{ py: 4, color: palette.muted, display: "flex", alignItems: "center", gap: 1 }}><Search size={16} />Sin placas registradas</Box>}
        />
      </Box>

      <PlacaModal open={modalOpen} placa={editando} onClose={() => { setModalOpen(false); setEditando(null); }} onSubmit={guardar} />
    </Box>
  );
}
